...
!function(t){function n(n,a){var c=n[o],f=c&&i[c];if(void 0===a)return f||e(n);if(f){if(a in f)return f[a];var s=u(a);if(s in f)return f[s]}return r.call(t(n),a)}function e(n,e,r){var c=n[o]||(n[o]=++t.uuid),f=i[c]||(i[c]=a(n));return void 0!==e&&(f[u(e)]=r),f}function a(n){var e={};return t.each(n.attributes||c,function(n,a){0==a.name.indexOf("data-")&&(e[u(a.name.replace("data-",""))]=t.zepto.deserializeValue(a.value))}),e}var i={},r=t.fn.data,u=t.camelCase,o=t.expando="Zepto"+ +new Date,c=[];t.fn.data=function(a,i){return void 0===i?t.isPlainObject(a)?this.each(function(n,i){t.each(a,function(t,n){e(i,t,n)})}):0 in this?n(this[0],a):void 0:this.each(function(){e(this,a,i)})},t.data=function(n,e,a){return t(n).data(e,a)},t.hasData=function(n){var e=n[o],a=e&&i[e];return a?!t.isEmptyObject(a):!1},t.fn.removeData=function(n){return"string"==typeof n&&(n=n.split(/\s+/)),this.each(function(){var e=this[o],a=e&&i[e];a&&t.each(n||a,function(t){delete a[n?u(this):t]})})},["remove","empty"].forEach(function(n){var e=t.fn[n];t.fn[n]=function(){var t=this.find("*");return"remove"===n&&(t=t.add(this)),t.removeData(),e.call(this)}})}(Zepto);
...
if ($dialogs.length) {
    var $view = $('html,body');
    $dialogs.filter('[hidden]').on('dialog:fit', function() {
        var $this = $(this);
        $this.not('.expand').css({
            top: '50%',
            left: '50%',
    ...
    $dialogs.on('dialog:enter', function(e, $source) {
        var $this = $(this),
            $overlay = $('<div class="dialog-overlay"></div>');
        $overlay.appendTo($body);
        $this.prop('hidden', false).appendTo($body);
        $this.trigger('dialog:fit');
        $this.find(focusable).first().focus();
        $focus = $this.find(focusable_class);
        $focus.length && $focus.focus();
        $view.css('overflow', 'hidden');
    });
    $win.on("resize", function() {
        $dialogs.trigger('dialog:fit');
    });
    $dialogs.on('dialog:exit', function(e, $source) {
        var $this = $(this),
            $overlay = $this.prev('.dialog-overlay');
        $overlay.length && $overlay.remove();
        $this.prop('hidden', true);
        $view.css('overflow', "");
    });
    $dialogs.find('form').on("submit", function() {
        $(this).closest('.dialog').trigger('dialog:exit');
        // return false;
    });
    $dialogs.each(function(i) {
        var $x = $('<a class="dialog-x js:exit" href=""><svg class="icon only" viewBox="0 0 24 24"><path d="M19,6.41L17.59,5L12,10.59L6.41,5L5,6.41L10.59,12L5,17.59L6.41,19L12,13.41L17.59,19L19,17.59L13.41,12L19,6.41Z"></path></svg></a>'),
            $overlay = $('<div class="dialog-overlay"></div>'),
            $this = $(this);
        $.each({
            'header': $this.children('header'),
            'title': $this.children('header').find('.title'),
            'body': $this.children('div'),
            'content': $this.children('div').children('div'),
        ...
        var $enter = ($this.data('jsEnter') || "").replace(/[:]/g, '\\$&'),
            $exit = $this.find('.js\\:exit');
        if ($enter && ($enter = $($enter)).length) {
            $enter.on("click", function(e) {
                $this.prop('hidden', true).trigger('dialog:enter', [$(this)]);
                return false;
            });
        }
        if ($exit.length) {
            $exit.on("click", function(e) {
                $this.trigger('dialog:exit', [$(this)]);
                return false;
            });
        }
    });
    ...
    $tabs.each(function(i) {
        ...
        $sections.each(function(j) {
            ...
            $a.on("click", function(e) {
                ...
                $this.trigger('tab:enter', [$source, $target]);
                $this.trigger('tab:exit', [$source, $old]);
                $this.trigger('tab:change', [$source, $target]);
                $dialogs.trigger('dialog:fit');
                return $href !== "";
            });
            $li.appendTo($ul);
...
if ($navs.length) {
    ...
    $doc.on("click", function() {
        $navs.trigger('nav:exit', [null, null]);
    });
}
    ...
    $layers.each(function() {
        ...
        $a.on("click", function() {
            var $source = $(this),
                ...
                $old = $c.filter('.active');
            if ($target.length) {
                $target.trigger('layer:enter', [$source, $target]);
                $target.trigger('layer:exit', [$source, $old]);
                $target.trigger('layer:change', [$source, $target]);
                return false;
            }
        });
    });
...
if ($aLayers.length) {
    $aLayers.each(function() {
        ...
        $this.on("click", function() {
            ...
            $target.trigger('layer:exit', [$source, $old]);
            $target.trigger('layer:enter', [$source, $next]);
            $target.trigger('layer:change', [$source, $next]);
            return false;
        });
    });
}
...
if ($forms.length) {
    $forms.on("submit", function() {
        $(this).trigger('form:' + (this.method.toLowerCase() === 'get' ? 'get' : 'set'));
    });
