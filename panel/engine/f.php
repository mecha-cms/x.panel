...
namespace _\lot\x\panel\type {
    function bar($value, $key) {
        if (isset($value['lot'])) {
            \_\lot\x\panel\h\p($value['lot'], 'bar');
        }
    ...
    function field($value, $key) {
        ...
        if (!\array_key_exists('title', $value) || false !== $value['title']) {
            $title = \_\lot\x\panel\h\title($value, -2, \To::title($key));
        ...
        foreach (['before', 'after'] as $v) {
            if (isset($value[$v])) {
                if (\is_string($value[$v])) {
                    ...
                    $icon = \_\lot\x\panel\h\icon($value[$v]['icon'] ?? [null, null]);
        ...
        if (isset($value['content'])) {
            ...
            $out[1] .= '<div><div class="lot' . ($before || $after ? ' lot:input' . (!empty($value['width']) ? ' width' : "") : "") . '">' . $before . \_\lot\x\panel\h\content($value['content']) . $after . '</div>' . \_\lot\x\panel\h\description($value) . '</div>';
        } else if (isset($value['lot'])) {
            $out[1] .= '<div>' . \_\lot\x\panel\h\lot($value['lot']) . '</div>';
        }
        \_\lot\x\panel\h\c($out[2], $value['tags'] ?? [], $tags);
        return new \HTML($out);
    }
    function fields($value, $key) {
        ...
        $append = "";
        $title = \_\lot\x\panel\h\title($value, 3);
        $description = \_\lot\x\panel\h\description($value);
        if (isset($value['content'])) {
            $out[1] .= \_\lot\x\panel\h\content($value['content']);
        } else if (isset($value['lot'])) {
            \_\lot\x\panel\h\p($value['lot'], 'field');
    ...
    function files($value, $key) {
        ...
        unset($lot);
        \_\lot\x\panel\h\c($out[2], $value['tags'] ?? [], [
            'count:' . $count => true,
            'lot' => true,
            'lot:file' => true
        ]);
    ...
    function folders($value, $key) {
        return \_\lot\x\panel\files($value, $key);
    }
    function form($value, $key) {
        ...
        if (isset($value['content'])) {
            ...
            $out[1] .= \_\lot\x\panel\h\lot($value['lot']);
        }
    ...
    function link($value, $key) {
        ...
        if ("" === $out[1]) {
            $out[1] = \_\lot\x\panel\h\title($value, -1, \To::title($key));
        }
    ...
    function page($value, $key) {
        ...
        $out[1] .= '<div><h3>' . \_\lot\x\panel\type\link([
            ...
            'url' => $value['url'] ?? null
        ], $key) . '</h3>' . \_\lot\x\panel\h\description($value, $date) . '</div>';
    ...
    function pages($value, $key) {
        ...
        unset($lot);
        \_\lot\x\panel\h\c($out[2], \array_replace([
            'count:' . $count => true,
            'lot' => true,
            'lot:page' => true
        ], $value['tags'] ?? []));
    ...
    function tabs($value, $key) {
        ...
        $tags['count:' . $count] = true;
        \_\lot\x\panel\h\c($out[2], $tags, $value['tags'] ?? []);
        return new \HTML($out);
    }
    function tasks($value, $key) {
        ...
        if (isset($value['content'])) {
            ...
            $out[1] .= \_\lot\x\panel\h\lot($value['lot'], null, $count);
            $tags['count:' . $count] = true;
        }
        if ($count > 0) {
            \_\lot\x\panel\h\c($out[2], $tags);
    ...
    function content($value, $key) {
        ...
        $title = \_\lot\x\panel\h\title($value, 2);
        $description = \_\lot\x\panel\h\description($value);
        $out = [
            0 => $value[0] ?? 'div',
    ...
    function type($value, $key) {
        ...
        if ($type = isset($value['type']) ? \strtr($value['type'], [
            ...
            if ($type && \function_exists($fn = __NAMESPACE__ . "\\type\\" . $type)) {
                ...
                $out .= \_\lot\x\panel\type\abort($value, $key, $fn);
            }
        } else {
            if (isset($value['content'])) {
                $out .= \_\lot\x\panel\type\content($value, $key);
