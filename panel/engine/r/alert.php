...
if ($_['i'] && $q = ($_GET['q'] ?? null)) {
    $GLOBALS['_']['alert']['info'][] = i('Search results for query %s', '<em>' . $q . '</em>') . ' <a class="f:r" href="' . $url->clean . '/1' . $url->query('&', ['q' => false]) . $url->hash . '" title="' . i('Exit search') . '">' . _\lot\x\panel\to\icon('M19,6.41L17.59,5L12,10.59L6.41,5L5,6.41L10.59,12L5,17.59L6.41,19L12,13.41L17.59,19L19,17.59L13.41,12L19,6.41Z')[0] . '</a>';
}

if (defined('DEBUG') && DEBUG && is_file($f = ENGINE . DS . 'log' . DS . 'error')) {
    $errors = _\lot\x\panel\from\path(trim(n(file_get_contents($f))));
