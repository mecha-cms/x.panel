...
if (is_file($f) && 'zip' === pathinfo($f, PATHINFO_EXTENSION)) {
    if ($zip = wapmorgan\UnifiedArchive\UnifiedArchive::open($f)) {
        ...
        foreach ($zip->getFileNames() as $v) {
            $x = \pathinfo($v = \strtr($v, '/', \D), \PATHINFO_EXTENSION);
            $self = \dirname($file) . \D . $v;
            // This prevents user(s) from uploading forbidden file(s)
            if ($x && false === \strpos($test_x, \P . $x . \P)) {
                $_['alert']['error'][$self] = ['File extension %s is not allowed.', '<code>' . $x . '</code>'];
            // This prevents user(s) from accidentally overwrite the existing file(s)
            } else if (\is_file($self)) {
                $_['alert']['error'][$self] = ['File %s already exists.', '<code>' . \x\panel\from\path($self) . '</code>'];
            } else {
                $_SESSION['_']['file'][$self] = 1;
                $_SESSION['_']['folder'][\rtrim(\dirname($self), \D)] = 1;
            }
        }
        if (!empty($_['alert']['error'])) {
            $_['alert']['error'][$file] = ['Package %s could not be extracted due to the previous errors.', '<code>' . x\panel\from\path($f) . '</code>'];
