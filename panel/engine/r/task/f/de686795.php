    ...
    if ($zip = wapmorgan\UnifiedArchive\UnifiedArchive::open($f)) {
        ...
        foreach ($zip->getFileNames() as $v) {
            ...
            // This prevents user(s) from accidentally overwrite the existing file(s)
                ...
                $_['alert']['error'][] = ['File %s already exists.', '<code>' . x\panel\from\path($ff) . '</code>'];
            } else {
        ...
        if (!empty($_['alert']['error'])) {
            $_['alert']['error'][] = ['Package %s could not be extracted due to the previous errors.', '<code>' . x\panel\from\path($f) . '</code>'];
            ...
            if (!empty($_['form']['lot']['let'])) {
                ...
                $_['alert']['success'][] = ['Package %s successfully extracted.', '<code>' . x\panel\from\path($f) . '</code>'];
            }
        }
        unset($zip);
    }
