...
Hook::set('do.blob.set', function($_) {
    ...
    if (!$error) {
        foreach (g(LOT . DS . $_['chop'][0], null, true) as $k => $v) {
...
$description .= '<p>' . i('Please note that this action will first delete all of your current layout files before replacing it with the new ones.') . ' ' . i('You may want to <a href="%s" target="_blank" title="%s">save a copy</a> of your current layout files before doing this.', [$url . $_['/'] . '/::f::/de686795/' . $_['chop'][0] . $url->query('&amp;', [
    'd' => 0,
